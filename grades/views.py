    ...
    def get_avgGrade(self):
        ...
        gradelist = StudentGrade.objects.filter(hukbun=s).filter(valid='유효').filter(
            Q(grade='A+') | Q(grade='A') | Q(grade='B+') | Q(grade='B') | Q(grade='C+') | Q(grade='C') | Q(
                grade='D+') | Q(grade='D') | Q(grade='F')).values_list('grade', flat=True)
        count = len(gradelist)
        for g in gradelist:
...
class MajorGradeLV(LoginRequiredMixin, ListView):
    ...
    def get_score_sum(self):
        s = self.request.user.hukbun
        sum = 0
        scorelist = StudentGrade.objects.filter(hukbun=s).filter(Q(eisu='컴과') | Q(eisu='전필')).filter(
            valid='유효').values_list('score', flat=True)
        for i in scorelist:
            sum = sum + int(i)
    ...
    def get_avgGrade(self):
        ...
        avg = 0.0
        sum = 0.0
        gradelist = StudentGrade.objects.filter(hukbun=s).filter(Q(eisu='컴과') | Q(eisu='전필')).filter(valid='유효').filter(
            Q(grade='A+') | Q(grade='A') | Q(grade='B+') | Q(grade='B') | Q(grade='C+') | Q(grade='C') | Q(
                grade='D+') | Q(grade='D') | Q(grade='F')).values_list('grade', flat=True)
        count = len(gradelist)
        for g in gradelist:
    ...
    def get_fake_grade_sum(self):
        s = self.request.user.hukbun
        fake_grade_sum = StudentGrade.objects.filter(hukbun=s).filter(Q(eisu='컴과') | Q(eisu='전필')).values_list('score',
                                                                                                               flat=True)
        sum = 0
        for i in fake_grade_sum:
            sum = sum + int(i)
...
class GeGradeLV(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        return StudentGrade.objects.filter(hukbun=test).filter(
            Q(eisu='M자') | Q(eisu='필수') | Q(eisu='역철') | Q(eisu='경사') | Q(eisu='체기') | Q(eisu='사회'))
    ...
    def get_fake_grade_sum(self):
        ...
        fake_grade_sum = StudentGrade.objects.filter(hukbun=s).filter(
            Q(eisu='M자') | Q(eisu='필수') | Q(eisu='역철') | Q(eisu='경사') | Q(eisu='체기') | Q(eisu='사회')).values_list(
            'score', flat=True)
        sum = 0
        for i in fake_grade_sum:
            sum = sum + int(i)
