...
class StudentHopePaser(KutisParser):
    def parse_item(self,url):
        self.driver.get(url)
        html = self.driver.page_source
        soup = BeautifulSoup(html, "html.parser")
        ...
        for td in resultTd:
            print("%d" % i, td)
            i += 1
        """Make Student Hope object with table Crawling data and dictionary."""
        # 진로구분
        ...
        Hopelist.append(hope3)
        return list

    def save_info(self, hukbun, infos):
        """Save info from student infomation list to database."""
        info_object = StudentHopeCareers(hukbun=hukbun,
                                         # 진로구분
                                         careers=infos['careers'],
                                         # 지망순위
                                         ranking=infos['ranking'],
                                         # 한자성명
                                         job=infos['job'],
                                         # 영문성명
...
class StudentGradePaser(KutisParser):
    ...
    def save_info(self, hukbun, infos):
        # 리스트를 저장
        for td in infos:
            info_object = StudentGrade(hukbun=hukbun,
                                       # 이수구분
                                       eisu=td[0],
                                       # 인증구분
                                       certification=td[1],
