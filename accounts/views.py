...
def agree(request):
    ...
    if request.method == 'POST':
        ...
        if agreeDataUsing:  # db에 데이터가 있으면?
            ...
            test.save_info(test.make_info_object(test.parse_info()))
            test.save_info(test.make_grade_object(hukbunToSave, test.parse_grade()))
            # TO-DO : 동의성공 창 띄우기
            # TO-DO : 디비에 데이터 저장하기
    ...
    if request.method == 'POST':
        ...
        if kutis_login_success:  # 쿠티스 로그인에 성공했다면
            ...
            if user:  # DB에 유저가 있다면 ----- 데이터 업데이트
                ...
                parser.make_grade_object(hukbun, parser.parse_grade())
                return redirect('index')
